...
bool ColumnReader::all_pages_encoded_by_dict(ColumnIteratorOptions iter_opts){
    ...
    bool all_dict = false;
    if (dict_page_decoder){ all_dict = dict_page_decoder->is_dict_encoding();}
    return all_dict;
}
...
Status ColumnReader::get_dict_data(std::set<string>& dict_words){
    if (encoding_info()->encoding() == DICT_ENCODING ){
        ...
        if (all_pages_encoded_by_dict(iter_opts)){
            ...
            std::stringstream ss;
            ss << "not all pages in column("<<_meta.column_id()<<") are dict encoded, "<<_path_desc.debug_string(); 
            return Status::InternalError(ss.str().c_str());
        }
        
    }else{
        std::stringstream ss;
        ss << "column("<<_meta.column_id()<<") is not dict encoded, "<<_path_desc.debug_string(); 
        return Status::InternalError(ss.str().c_str());
    }
